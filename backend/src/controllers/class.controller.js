...
// HELPER: Check if trainer is available (no time conflicts)
const checkTrainerAvailability = async (trainer_id, class_date, start_time, end_time, excludeClassId = null) => {
  ...
  const whereClause = {
    ...
    start_time: { [Op.lt]: end_time },    // Class starts before our end time
    end_time: { [Op.gt]: start_time }     // Class ends after our start time
  };
    ...
    if (trainer_id) {
      ...
      if (!availability.available) {
        return res.status(409).json({ 
          message: `This trainer is not available at this time. They have a class from ${availability.conflictTime} (${availability.conflictClass}).` 
        });
      }
    }
